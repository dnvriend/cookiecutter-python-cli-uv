...
- **Slash Commands**: Quick CLI invocations (e.g., `/{{cookiecutter.project_slug}}:help`)
- **Skills**: Comprehensive documentation with progressive disclosure (e.g., `/skill-{{cookiecutter.project_slug}}`)
...
**File**: `plugins/{{cookiecutter.project_slug}}/.claude-plugin/plugin.json`
...
**Location**: `plugins/{{cookiecutter.project_slug}}/commands/`
...
**Adding New Commands**:
1. Create `plugins/{{cookiecutter.project_slug}}/commands/<command>.md`
...
/{{cookiecutter.project_slug}}:command "argument"
```

## Skills

**Location**: `plugins/{{cookiecutter.project_slug}}/skills/{{cookiecutter.project_slug}}/SKILL.md`
...
Or:
```
Use the skill-{{cookiecutter.project_slug}} to help me understand this tool
```
...
1. **Update the slash command** (optional):
   - Create `plugins/{{cookiecutter.project_slug}}/commands/<command>.md`
...
3. `plugins/{{cookiecutter.project_slug}}/.claude-plugin/plugin.json` → `version`
...
# Check plugin
cat plugins/{{cookiecutter.project_slug}}/.claude-plugin/plugin.json | jq '.'

# List commands
ls plugins/{{cookiecutter.project_slug}}/commands/

# Check skill
cat plugins/{{cookiecutter.project_slug}}/skills/{{cookiecutter.project_slug}}/SKILL.md
```

### Test in Claude Code
1. Open your project in Claude Code
2. Try slash command: `/{{cookiecutter.project_slug}}:help`
3. Try skill: `/skill-{{cookiecutter.project_slug}}`
